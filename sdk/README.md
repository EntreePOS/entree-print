...
`render()` can return optional `artifact` drawing data from the service for future coordinated transfer; the SDK currently retains it without automatically importing it elsewhere. See [portable preparation](../API_V2_IMPLEMENTATION.md) for the HTTP contract and ownership limits.
