...
- Subscribe through the production SDK after a printer/job checkpoint. Seed two job versions between snapshot and replay, forward the actual SSE bytes through the stdio bridge until catch-up, and deliberately end that response. Reopen service storage, save a third version and let the SDK resume its original cursor. Chinese metadata and version order survive; no render, job submission or backend delivery occurs. The bridge buffers each finite catch-up response; separate stream tests cover live disconnect cleanup, and SDK tests split UTF-8/line boundaries.
- Submit one receipt with trailing cut/beep actions through the actual SDK and Chromium preparation, lose both acceptance ACKs, finish the three fixture deliveries and reopen storage with expired previews. The same ticket recovers its original job and phase outcomes using identical bytes, one render and no extra delivery. Changing actions under the same key is rejected. Delivery is a fixture; no physical beep or cut is performed.
