...
The current suite passes 374 .NET tests, 95 core SDK tests and nine native SQLite tests locally. Nine integration scenarios run the actual SDK against ASP.NET TestServer, real Chromium preparation and the durable ledger. They cover lost acknowledgements, independent offline destinations, replay, events and ordered actions. Browser and native storage tests cover reload/process exit and concurrent dispatch. Prior [virtual spooler evidence](../samples/BetaSmoke/README.md) and [process termination recovery](../samples/RecoverySmoke/README.md) retain their stated scope. Exact tested source and installer evidence are recorded in [PACKAGE_VERIFICATION.md](../PACKAGE_VERIFICATION.md).
